...
export const createTransaction = async (
  client,
  { outlet_id, receipt_number, total_amount, notes },
) => {
  const { rows } = await client.query(
...
export const createTransactionItems = async (client, transactionId, items) => {
  const valuesWithSubtotal = items
    .map((_, i) => {
      const base = i * 5;
  ...
  const paramsWithSubtotal = items.flatMap(
    ({ menu_item_id, quantity, unit_price, subtotal }) => [
      transactionId,
      menu_item_id,
      quantity,
      unit_price,
