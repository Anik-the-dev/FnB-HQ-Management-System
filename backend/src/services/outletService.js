...
export const assignMenuItemToOutlet = async (outletId, { menu_item_id, override_price }) => {
  await getOutletById(outletId);
  ...
  // Assign menu item
  const assignment = await outletRepo.assignMenuItem({ outlet_id: outletId, menu_item_id, override_price });
...
export const updateOutletMenuItemAssignment = async (outletId, menuItemId, fields) => {
  const assignment = await outletRepo.updateMenuItemAssignment(outletId, menuItemId, fields);
  if (!assignment) {
...
export const removeOutletMenuItemAssignment = async (outletId, menuItemId) => {
  const removed = await outletRepo.removeMenuItemAssignment(outletId, menuItemId);
  if (!removed) {
    const err = new Error('Assignment not found');
    err.statusCode = 404;
    throw err;
